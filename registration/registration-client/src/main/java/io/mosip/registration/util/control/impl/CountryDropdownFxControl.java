...
public class CountryDropdownFxControl extends FxControl {
    ...
    public CountryDropdownFxControl () {
    ...
    public FxControl build(UiFieldDTO uiFieldDTO) {
        ...
        if(subTypeLangCode != null) {
            ...
            for (Map.Entry<Integer, String> entry : GenericController.hierarchyLevels.get(subTypeLangCode).entrySet()) {
    ...
    public List<GenericDto> getPossibleValues(String langCode) {
        ...
        if(GenericController.currentHierarchyMap.containsKey(uiFieldDTO.getGroup())) {
            isHierarchical = true;
            Map.Entry<Integer, String> parentEntry = GenericController.currentHierarchyMap.get(uiFieldDTO.getGroup())
        ...
        if(uiFieldDTO.getId().equals("nationality")){
            List<GenericDto> list = masterSyncService.getFieldValues(fieldSubType, langCode, isHierarchical);
           return list.stream().filter(e->!e.getCode().equalsIgnoreCase("ZA")).collect(Collectors.toList());
    ...
    public void setListener(Node node) {
        ...
        fieldComboBox.getSelectionModel().selectedItemProperty().addListener((options, oldValue, newValue) -> {
            ...
            if (isValid()) {
                ...
                // Auto set nationality when residenceStatus = NFR
                if (uiFieldDTO.getId().equalsIgnoreCase(RegistrationConstants.RESIDENCE_STATUS)) {

                    FxControl nationalityControl = GenericController.getFxControlMap().get(RegistrationConstants.NATIONALITY);

                    if (nationalityControl != null) {

                        ComboBox<GenericDto> nationalityCombo =
                                (ComboBox<GenericDto>) nationalityControl.getNode().lookup(RegistrationConstants.HASH_NATIONALITY);

                        if (nationalityCombo != null) {


                            List<GenericDto> fullList = masterSyncService.getFieldValues(
                                    nationalityControl.getUiSchemaDTO().getId(),
                                    nationalityControl.getUiSchemaDTO().getLabel().keySet().stream().findFirst().get(),
                                    false
                            );
                            ...
                            if (RegistrationConstants.ATTR_NON_FORINGER.equalsIgnoreCase(selectedCode)) {
                                ...
                                nationalityCombo.setItems(FXCollections.observableArrayList(fullList));


                                fullList.stream()
                                        .filter(dto -> RegistrationConstants.ATTR_SOUTH_AFRICAN_CODE.equalsIgnoreCase(dto.getCode()))
                                        .findFirst()
                                        .ifPresent(dto -> nationalityCombo.getSelectionModel().select(dto));

                                // disable nationality field (grey out)
                                nationalityCombo.setEditable(false);
                                nationalityCombo.setMouseTransparent(true);

                            } else {

                                //  Remove ZA for non-citizen
                                List<GenericDto> filteredList = fullList.stream()
                                        .filter(dto -> !RegistrationConstants.ATTR_SOUTH_AFRICAN_CODE.equalsIgnoreCase(dto.getCode()))
                                        .collect(Collectors.toList());

                                nationalityCombo.setItems(FXCollections.observableArrayList(filteredList));

                                nationalityCombo.setDisable(false);
                                ...
                                Label nationalityMessageLabel =
                                        (Label) nationalityControl.getNode().lookup(RegistrationConstants.HASH_NATIONALITY_MSG);

                                if (nationalityMessageLabel != null) {
                                    nationalityMessageLabel.setText("");
                                }
                            }
                        }
                    }
    ...
    private void refreshNextHierarchicalFxControls() {
        if(GenericController.currentHierarchyMap.containsKey(uiFieldDTO.getGroup())) {
            Map.Entry<Integer, String> nextEntry = GenericController.currentHierarchyMap.get(uiFieldDTO.getGroup())
